...
class AccountAppStateResilienceUnitTests(unittest.TestCase):
    ...
    def test_account_app_support_view_renders_support_hub(self):
        ...
        self.assertIn("Что написать", html)
        self.assertIn("Устройство и приложение", html)
        self.assertIn("Скопировать ID клиента", html)
        self.assertIn("Открыть инструкцию", html)
        self.assertIn("/account-app/?view=instructions&amp;embed=1", html)
        self.assertIn("vx-account-bot", html)
        self.assertIn("vx-bot-card", html)
        self.assertNotIn('href="/instructions/"', html)
        self.assertNotIn("account-surface", html)
    ...
    def test_account_app_install_detects_device_and_lists_requested_apps(self):
        ...
        self.assertIn("Если приложение уже установлено", html)
    ...
    def test_open_app_auto_import_preserves_nested_encoded_url(self):
        ...
        shadowrocket_url = "shadowrocket://add/sub://aHR0cHM6Ly92eGNsb3VkLnJ1L2FjY291bnQvZmVlZC9mZWVkLXRva2VuLw==?remarks=VXcloud"
        ...
        self.assertIn("renderState();\n            openCurrent(true);", html)
        self.assertIn("const useHiddenLauncher", html)
        self.assertIn("launchFrame.src = item.url", html)
        self.assertIn("openCurrent(true)", html)
        self.assertIn("currentLink.addEventListener('click'", html)
        self.assertIn("leftPage || document.hidden || index !== launchedIndex", html)
        self.assertIn("waitingForFocus = true", html)
        self.assertIn("function markRegainedFocus()", html)
        self.assertIn("lostFocus && !regainedFocus && !document.hasFocus()", html)
