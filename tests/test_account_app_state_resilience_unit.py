...
class AccountAppStateResilienceUnitTests(unittest.TestCase):
    ...
    def test_open_app_auto_import_preserves_nested_encoded_url(self):
        ...
        self.assertIn("импорт продолжится автоматически", html)
        self.assertIn("попробуем открыть его еще раз", html)
        self.assertIn("streisandWarmupPending", html)
        self.assertIn("vxcloud_streisand_network_ready_v1", html)
        self.assertIn("window.location.href = 'streisand://'", html)
