...
class BotMainMenuUnitTests(unittest.IsolatedAsyncioTestCase):
    async def test_register_exposes_current_customer_slash_commands(self):
        app = FakeApp()
        settings = SimpleNamespace(
            card_payment_amount_minor=24900,
            card_payment_currency="RUB",
        ...
        bot = VPNBot(app=app, settings=settings, db=FakeDB(), xui=FakeXUI())
    ...
    async def test_known_stale_russian_cms_overrides_fall_back_to_current_defaults(self):
        bot = make_bot()
        bot._cms_content.update(
            {
                ...
                "my_configs_list_template": "Ваш доступ VXcloud\n\nID: {client_code}\n\nВаши устройства:\n\n{items}",
                "my_configs_item_template": "{index}. {name}\nДействует до: {expires_at}\nСтатус: {status}",
                "my_configs_empty_message": "Ваш доступ VXcloud\n\nID: {client_code}\n\nВаши устройства:\n\nСписок устройств пока пуст.",
                "copy_link_hint": "Нажмите «Скопировать ссылку», затем откройте приложение и выберите импорт из буфера обмена.",
                "reminder_expired_message": "Истёк конфиг VXcloud: {name}\nДействовал до: {expires_at}\n\nИспользуйте /buy для продления.",
                "reminder_1d_message": "Напоминание: конфиг VXcloud скоро истекает",
                "reminder_3d_message": "Напоминание: конфиг VXcloud истекает менее чем через 3 дня",
            }
        )

        self.assertIn("История обращений: «📱 Кабинет» → «Поддержка».", bot._support_received_text(ticket_id=77, client_code="VX-000123"))
        self.assertIn("{summary}", bot._content_text("my_configs_list_template", "{summary}"))
        self.assertIn("{status_icon}", bot._content_text("my_configs_item_template", "{status_icon}"))
        self.assertIn("Пока нет устройств.", bot._content_text("my_configs_empty_message", "Пока нет устройств."))
        self.assertIn("ссылку подписки", bot._content_text("copy_link_hint", "ссылку подписки"))
        self.assertIn("Доступ VXcloud истёк", bot._content_text("reminder_expired_message", "Доступ VXcloud истёк"))
        self.assertIn("Доступ VXcloud скоро закончится", bot._content_text("reminder_1d_message", "Доступ VXcloud скоро закончится"))
        self.assertIn("Доступ VXcloud закончится", bot._content_text("reminder_3d_message", "Доступ VXcloud закончится"))
    ...
    async def test_buy_copy_names_mini_app_as_card_checkout_surface(self):
        ...
        self.assertIn("Один доступ рассчитан на одно устройство", text)
    ...
    async def test_buy_with_active_access_is_app_first_for_additional_access(self):
        ...
        self.assertIn("для другого телефона или компьютера", text)
    ...
    async def test_payment_ready_markups_keep_app_primary_and_qr_only(self):
        ...
        paid_markup = bot._post_payment_ready_markup(42, account_url, "https://vxcloud.ru/account/feed/token/")
        renew_markup = bot._renew_success_markup(42, account_url, "https://vxcloud.ru/account/feed/token/")

        self.assertEqual(paid_markup.inline_keyboard[0][0].text, "Подключить")
        self.assertIn("https://vxcloud.ru/open-app/?mode=ios-auto", paid_markup.inline_keyboard[0][0].url)
        self.assertEqual(paid_markup.inline_keyboard[1][0].text, "Скопировать ссылку")
        self.assertEqual(paid_markup.inline_keyboard[1][0].api_kwargs["copy_text"]["text"], "https://vxcloud.ru/account/feed/token/")
        self.assertEqual(paid_markup.inline_keyboard[2][0].text, "QR и доступ")
        self.assertEqual(paid_markup.inline_keyboard[2][0].web_app.url, "https://vxcloud.ru/account-app/config/42/?embed=1")
        self.assertEqual(paid_markup.inline_keyboard[3][0].text, "Настроить вручную")
        self.assertEqual(paid_markup.inline_keyboard[3][0].web_app.url, "https://vxcloud.ru/account-app/install/42/?embed=1")
        self.assertEqual(len(paid_markup.inline_keyboard), 4)
        self.assertEqual(renew_markup.inline_keyboard[0][0].text, "Подключить")
        self.assertIn("https://vxcloud.ru/open-app/?mode=ios-auto", renew_markup.inline_keyboard[0][0].url)
        self.assertEqual(renew_markup.inline_keyboard[1][0].text, "Скопировать ссылку")
        self.assertEqual(renew_markup.inline_keyboard[2][0].text, "QR и доступ")
        self.assertEqual(renew_markup.inline_keyboard[3][0].text, "Настроить вручную")
        self.assertEqual(len(renew_markup.inline_keyboard), 4)
    ...
    async def test_send_config_actions_are_mini_app_first(self):
        ...
        self.assertIn("\u041e\u0441\u043d\u043e\u0432\u043d\u043e\u0439 \u0441\u043f\u043e\u0441\u043e\u0431 \u043f\u043e\u0434\u043a\u043b\u044e\u0447\u0435\u043d\u0438\u044f", text)
        self.assertIn("Streisand", text)
        ...
        self.assertIn("https://vxcloud.ru/open-app/?mode=ios-auto", markup.inline_keyboard[0][0].url)
        ...
        self.assertEqual(markup.inline_keyboard[3][0].text, "Настроить вручную")
        self.assertEqual(markup.inline_keyboard[3][0].web_app.url, "https://vxcloud.ru/account-app/install/42/?embed=1")
        self.assertEqual(markup.inline_keyboard[4][0].web_app.url, "https://vxcloud.ru/account-app/renew/?subscription_id=42&embed=1")
        self.assertEqual(markup.inline_keyboard[5][0].text, "📖 Как подключить")
        self.assertEqual(markup.inline_keyboard[5][0].callback_data, "nav|menu_instructions|_")
    ...
    async def test_send_config_fallback_connection_link_label_is_russian(self):
        ...
        self.assertEqual(message.replies[-1][1].inline_keyboard[0][0].text, "Подключить")
    ...
    async def test_trial_success_markup_uses_mini_app_setup_first(self):
        ...
        markup = bot._trial_success_markup(42, "https://vxcloud.ru/account/feed/token/")
    ...
    async def test_trial_success_copy_is_compact_and_shows_expiry(self):
        ...
        self.assertIn("Откройте QR и доступ или инструкцию.", text)
    ...
    async def test_my_vpn_list_is_compact_selector_only(self):
        bot = make_bot()
    ...
    async def test_my_vpn_list_sorts_active_first_and_shows_status_badges(self):
        ...
        self.assertEqual(markup.inline_keyboard[2][0].text, "3. ⚠️ Old phone")
        self.assertEqual(len(markup.inline_keyboard), 3)

    async def test_my_vpn_list_keeps_expired_devices_compact(self):
        bot = make_bot()
    ...
    async def test_instructions_command_restores_menu_and_opens_hub(self):
        bot = make_bot()
        message = FakeMessage()
        context = SimpleNamespace(user_data={"support_wait_message": True})

        await bot.instructions(make_update(message), context)
    ...
    async def test_support_command_restores_menu_and_opens_support_hub(self):
        bot = make_bot()
        message = FakeMessage()
        context = SimpleNamespace(user_data={"rename_wait_subscription_id": 42})

        await bot.support(make_update(message), context)
        ...
        self.assertEqual(message.replies[-1][1].inline_keyboard[0][0].callback_data, "act|support_start|_")

    async def test_open_app_command_restores_menu_and_tracks_slash_source(self):
        db = FakeDB()
        bot = make_bot(db)
        message = FakeMessage()
        context = SimpleNamespace(user_data={"support_wait_message": True})

        await bot.open_app(make_update(message), context)

        self.assertNotIn("support_wait_message", context.user_data)
        self.assertIsInstance(message.replies[0][1], ReplyKeyboardMarkup)
        self.assertEqual(db.events[-1]["event_name"], "open_app")
        self.assertEqual(db.events[-1]["metadata"], {"source": "slash_command"})
        text, markup = message.replies[-1]
        self.assertIn("\u043a\u0430\u0431\u0438\u043d\u0435\u0442 VXcloud", text)
        self.assertEqual(markup.inline_keyboard[0][0].web_app.url, "https://vxcloud.ru/account-app/?embed=1")
        self.assertEqual(markup.inline_keyboard[1][0].url, "https://vxcloud.ru/account/")
    ...
    async def test_config_card_delete_uses_confirmation_callback(self):
        ...
        self.assertEqual(markup.inline_keyboard[3][0].text, "Настроить вручную")
        self.assertEqual(markup.inline_keyboard[3][0].web_app.url, "https://vxcloud.ru/account-app/install/42/?embed=1")
        self.assertEqual(markup.inline_keyboard[4][0].text, "QR")
        self.assertEqual(markup.inline_keyboard[4][0].callback_data, "act|cfg_qr:42|_")
        self.assertEqual(markup.inline_keyboard[4][1].text, "🔄 Продлить")
        self.assertEqual(markup.inline_keyboard[4][1].web_app.url, "https://vxcloud.ru/account-app/renew/?subscription_id=42&embed=1")
        self.assertEqual(markup.inline_keyboard[5][0].text, "\u041f\u0435\u0440\u0435\u0438\u043c\u0435\u043d\u043e\u0432\u0430\u0442\u044c")
        self.assertEqual(markup.inline_keyboard[5][1].text, "\u0423\u0434\u0430\u043b\u0438\u0442\u044c")
        self.assertEqual(markup.inline_keyboard[5][1].callback_data, "act|cfg_delete_request:42|_")
        self.assertEqual(markup.inline_keyboard[6][0].text, "Назад")
        self.assertEqual(markup.inline_keyboard[6][0].callback_data, "act|cfg_back|_")
    ...
    async def test_expired_config_card_puts_renewal_first(self):
        ...
        self.assertEqual(markup.inline_keyboard[3][0].text, "Настроить вручную")
        self.assertEqual(markup.inline_keyboard[3][0].web_app.url, "https://vxcloud.ru/account-app/install/42/?embed=1")
        self.assertEqual(markup.inline_keyboard[4][0].text, "🔄 Продлить")
        self.assertEqual(markup.inline_keyboard[4][0].web_app.url, "https://vxcloud.ru/account-app/renew/?subscription_id=42&embed=1")
        self.assertEqual(markup.inline_keyboard[5][0].text, "QR")
        self.assertEqual(markup.inline_keyboard[5][0].callback_data, "act|cfg_qr:42|_")
        self.assertEqual(markup.inline_keyboard[6][1].callback_data, "act|cfg_delete_request:42|_")
        self.assertEqual(markup.inline_keyboard[7][0].text, "Назад")
        self.assertEqual(markup.inline_keyboard[7][0].callback_data, "act|cfg_back|_")
    ...
    async def test_config_card_text_is_compact_and_does_not_print_access_link(self):
        ...
        self.assertNotIn("ссылку подключения", text)

    async def test_config_card_text_raw_link_fallback_stays_action_oriented(self):
        db = FakeDB()
        db.subscriptions[42] = active_subscription(42, name="Phone")
        db.subscriptions[42]["feed_token"] = ""
        bot = make_bot(db)
        bot._subscription_feed_url = AsyncMock(return_value=None)

        text, primary_link, raw_link = await bot._config_card_text(123, db.subscriptions[42], client_code="VX-000123")

        self.assertEqual(primary_link, raw_link)
        self.assertIn("Основной способ подключения", text)
        self.assertIn("VPN-приложение", text)
        self.assertNotIn("QR и кнопка", text)
        self.assertNotIn("ссылку подключения", text)
        self.assertNotIn("vless://", text)
    ...
    async def test_inline_action_during_support_input_cancels_state_and_restores_menu(self):
        db = FakeDB()
        db.subscriptions[42] = active_subscription(42, name="Phone")
        bot = make_bot(db)
        query = FakeCallbackQuery("act|cfg_open:42|_")
        context = SimpleNamespace(user_data={"support_wait_message": True})

        await bot.inline_callback(make_callback_update(query), context)

        self.assertNotIn("support_wait_message", context.user_data)
        self.assertIsInstance(query.message.replies[0][1], ReplyKeyboardMarkup)
        self.assertIn("\u0412\u0432\u043e\u0434 \u043e\u0442\u043c\u0435\u043d\u0435\u043d", query.message.replies[0][0])
        self.assertIn("\u0423\u0441\u0442\u0440\u043e\u0439\u0441\u0442\u0432\u043e: Phone", query.edits[-1][0])
    ...
    async def test_qr_action_uses_subscription_feed_url_not_raw_vless(self):
        ...
        self.assertEqual(query.message.photos[0][2].inline_keyboard[0][0].text, "Подключить")
        self.assertEqual(query.message.photos[0][2].inline_keyboard[1][0].text, "Скопировать ссылку")
        self.assertEqual(query.message.photos[0][2].inline_keyboard[2][0].text, "QR и доступ")
        self.assertEqual(query.message.photos[0][2].inline_keyboard[3][0].text, "Настроить вручную")
    ...
    async def test_reminder_defaults_use_access_wording_not_config_wording(self):
        ...
        self.assertEqual(markup.inline_keyboard[1][0].text, "📱 QR и доступ")
    ...
    async def test_renew_offer_requires_explicit_choice_for_multiple_subscriptions(self):
        ...
        self.assertIn("Продление добавит срок только к нему", message.replies[-1][0])
        markup = message.replies[-1][1]
        self.assertEqual(markup.inline_keyboard[0][0].text, "1. ⚠️ Phone")
        self.assertEqual(markup.inline_keyboard[1][0].text, "2. ⚠️ Laptop")
    ...
    async def test_renew_copy_names_mini_app_as_card_checkout_surface(self):
        ...
        text = message.replies[-1][0]
        self.assertIn("Продление добавит срок этому устройству", text)
        self.assertIn("Картой", text)
        self.assertIn("Stars", text)
        self.assertNotIn("на сайте", text)
    ...
    async def test_support_hub_offers_quick_message_and_mini_app(self):
        ...
        self.assertEqual(markup.inline_keyboard[0][0].text, "✍️ Написать в поддержку")
    ...
    async def test_show_support_hub_uses_contextual_inline_markup(self):
        ...
        self.assertIn("Мы ответим здесь, в Telegram", message.replies[0][0])
        self.assertIn("устройство, что не работает", message.replies[0][0])
        ...
        self.assertEqual(len(message.replies[0][1].inline_keyboard), 2)
    ...
    async def test_support_start_hides_main_menu_and_shows_cancel_keyboard(self):
        ...
        self.assertIn("одним сообщением", query.message.replies[-1][0])
        self.assertIn("устройство", query.message.replies[-1][0])
    ...
    async def test_support_start_replaces_existing_text_input_state(self):
        bot = make_bot()
        query = FakeCallbackQuery("act|support_start|_")
        context = SimpleNamespace(user_data={"rename_wait_subscription_id": 42})
    ...
    async def test_support_message_submission_restores_main_menu(self):
        ...
        self.assertIn("Поддержка", message.replies[-1][0])
        self.assertIn("\u0412\u0430\u0448 ID: VX-000123", message.replies[-1][0])
    ...
    async def test_support_received_copy_is_ticket_oriented_and_menu_safe(self):
        ...
        self.assertIn("История обращений", text)
